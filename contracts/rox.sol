...
contract ROX is ERC20, Ownable {
    ...
    ExtraFeeReceiver public extraFeeReceiver;
    DividendTracker public dividendTracker;
    bool private swapping;
    bool public enableHoldDividend;

    address public constant ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public constant BIN = 0x55d398326f99059fF775485246999027B3197955;
    address public constant USDT = 0x55d398326f99059fF775485246999027B3197955;
    address public constant tokenOwner = 0x5D1cc34C0d54Cf2256672A30849B579D4eae0ef1;
    address public constant node = 0x19013344A3F24D937e87aD94e2602c177Ff1A2e9;
    address public constant pool = 0x55d398326f99059fF775485246999027B3197955;
    address public com = 0x55d398326f99059fF775485246999027B3197955;
    address public fund = 0x55d398326f99059fF775485246999027B3197955;
    ...
    uint public minRewardBalance = 100 * 1e18;

    uint public startTime;
    uint public freezeTime = 60;
    uint256 public gasForProcessing = 200000;
    uint256 public lastProcessedIndex;
    uint public minLpValue = 10 * 1e18;

    uint public extraFee = 250;
    ...
    constructor() ERC20("ROX", "ROX") {
        require(address(this) > BIN);
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(ROUTER);
         // Create a uniswap pair for this new token
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), BIN);

        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = _uniswapV2Pair;
        isPair[_uniswapV2Pair] = true;
        isRouter[ROUTER] = true;
        tokenReceiver = new TokenReceiver(BIN);
        extraFeeReceiver = new ExtraFeeReceiver();

        dividendTracker = new DividendTracker(BIN);
    ...
    function setCom(address _com) external onlyOwner {
        require(_com != address(0));
        com = _com;
    }
    ...
    function setMinRewardBalance(uint _minRewardBalance) external onlyOwner {
    ...
    function setMinLpValue(uint256 _minLpValue) external onlyOwner {
    ...
    function updateLpInfo(address[] memory users) external onlyOwner {
    ...
    function setBuyFees(uint _buyNodeFee, uint _buyLpFee, uint _buyFundFee) external onlyOwner {
        require(_buyNodeFee + _buyLpFee + _buyFundFee < 400, "fee too hight");
        buyNodeFee = _buyNodeFee;
        buyLpFee = _buyLpFee;
        buyFundFee = _buyFundFee;
    }

    function setSellFees(uint _sellNodeFee, uint _sellLpFee, uint _sellFundFee) external onlyOwner {
        require(_sellNodeFee + _sellLpFee + _sellFundFee < 400, "fee too hight");
    ...
    function _transfer(
        ...
        if( extraFeeBalance >= minSwapToken &&
            !swapping &&
            !isPair[from]
        ) {
            swapAndDividendExtraFee(minSwapToken);
        }
        ...
        if(!swapping && enableHoldDividend && takeFee) {
    ...
    function _tokenTransfer(
        ...
        if(takeFee) {
            ...
            if(isPair[sender]) { //buy
                require(startTime > 0);
                if (removeLPLiquidity == 0) {
                    feeToThis = buyLpFee + buyNodeFee + buyFundFee;
                ...
                require(startTime > 0);
                if (addLPLiquidity == 0) {
                    feeToThis = sellNodeFee + sellLpFee + sellFundFee;
                    uint extraFeeAmount;
        ...
        returns (uint256 rOther, uint256 rThis, uint256 balanceOther)
    {
        ...
        balanceOther = IERC20(BIN).balanceOf(uniswapV2Pair);
    }
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = buyLpFee + buyNodeFee + buyFundFee;
        uint totalSellShare = sellLpFee + sellNodeFee + sellFundFee;
        ...
        uint left = tokenAmount - amountToLp;
        if (left > 0) {
            uint256 binReceived = IERC20(BIN).balanceOf(address(tokenReceiver));
            swapTokensForBIN(left, address(tokenReceiver));
            binReceived = IERC20(BIN).balanceOf(address(tokenReceiver)) - binReceived;
            uint leftShare = buyNodeFee + buyFundFee + sellNodeFee + sellFundFee;
            uint toNode = binReceived * (buyNodeFee + sellNodeFee) / leftShare;
            IERC20(BIN).transferFrom(address(tokenReceiver), node, toNode);
            try INode(node).rewardTo(toNode) {} catch {}
            IERC20(BIN).transferFrom(address(tokenReceiver), fund, binReceived - toNode);
        }  
    }
    ...
    function swapAndDividendExtraFee(uint256 tokenAmount) private lockTheSwap {
        ...
        swapAndLiquify(tokenAmount / 5);

        uint256 initialBalance = IERC20(BIN).balanceOf(address(tokenReceiver));
        swapTokensForBIN(tokenAmount / 5 * 4, address(tokenReceiver));
        uint256 newBalance = (IERC20(BIN).balanceOf(address(tokenReceiver))).sub(initialBalance);
        IERC20(BIN).transferFrom(address(tokenReceiver), address(this), newBalance);
        uint quarter = newBalance / 4;
        IERC20(BIN).transfer(node, quarter);
        try INode(node).rewardTo(quarter) {} catch {}

        IERC20(BIN).transfer(com, quarter);
        IERC20(BIN).transfer(address(dividendTracker), quarter);
        dividendTracker.distributeETHDividends(quarter);
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
       // split the contract balance into halves
        ...
        // swap tokens for BIN
        swapTokensForBIN(half, address(tokenReceiver));

        // how much BIN did we just swap into?
        uint256 newBalance = (IERC20(BIN).balanceOf(address(tokenReceiver))).sub(initialBalance);
        IERC20(BIN).transferFrom(address(tokenReceiver), address(this), newBalance);
    ...
    function swapTokensForBIN(uint256 tokenAmount, address to) private {
        // generate the uniswap pair path of token -> BIN
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = BIN;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 BINAmount) private {
        // add the liquidity
    ...
    function process(uint256 gas) public {
        ...
        uint256 rewards = IERC20(BIN).balanceOf(address(this));
        ...
        uint valueOfPair = IERC20(BIN).balanceOf(uniswapV2Pair);
        ...
        while(gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            if (!excludeLpProvider[account]
                && share * valueOfPair / IUniswapV2Pair(uniswapV2Pair).totalSupply() >= minLpValue
                && lpAmountOf[account] + diff >= share) 
            {
                IERC20(BIN).transfer(account, share * rewards / totalShare);    
            }
